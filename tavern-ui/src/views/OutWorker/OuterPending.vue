<template>
  <div class="page-container">
    ...
    <!--工具栏-->
    <div class="toolbar" style="float:left;padding-top:10px;padding-left:15px;" v-if="sys_outer_pending_view">
      ...
      <el-form :inline="true" :model="filters" :size="size" align="left">
        ...
        <el-form-item>
          <kt-button icon="fa fa-search" :label="$t('action.search')" v-if="sys_outer_pending_view" type="primary"
                     @click="findPage(null)"/>
        </el-form-item>
      </el-form>
    </div>
    ...
    <el-table :data="tableData" stripe stripe height="500" size="mini" style="width: 100%;"
      ...
      <el-table-column label="操作"
        ...
        <template slot-scope="scope">
          <kt-button icon="fa fa-gears" label="流程日志" type="primary" v-if="sys_outer_pending_flow"
                     @click="showWorkFlow(scope.row)"/>
          <kt-button icon="fa fa-plus" label="添加备注" type="primary"
                     v-if="sys_outer_pending_addlog && scope.row.curOperatorName == userName "
                     @click="handleLog(scope.row)"/>
          <kt-button icon="fa fa-money" label="退款审批" type="primary"
                     v-if="scope.row.curNodeName == '退款审批' && sys_outer_pending_drawback && scope.row.curOperatorName == userName"
                     @click="handleDrawBack(scope.row)"/>
          <kt-button icon="fa fa-arrow-right" label="下一步" type="primary"
                     v-if="sys_outer_pending_next && scope.row.curOperatorName == userName"
  ...
  export default {
    ...
    data() {
      return {
        ...
        showRefund: false,
        showHandler: false,
        showNextOperator: true,
        handlerDict: [],
        data: {},
        userName: '',
        sys_outer_pending_view: false,
        sys_outer_pending_flow: false,
    ...
    created() {
      this.sys_outer_pending_view = hasPermission('sys:outer:pending:view')
      this.sys_outer_pending_flow = hasPermission('sys:outer:pending:flow')
      this.sys_outer_pending_addlog = hasPermission('sys:outer:pending:addlog')
